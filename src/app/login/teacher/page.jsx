...
export default function TeacherLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (user.role !== "teacher") throw new Error("Unauthorized role");
      toast.success(`Welcome back, ${user.name}!`);
      router.push("/teacher");
    } catch (err) {
      toast.error("Invalid teacher credentials. Use teacher/password");
    } finally {
        ...
        <div className="bg-card border border-pal-navy/10 p-8 rounded-3xl shadow-2xl">
          <div className="text-center mb-8">
            <div className="mx-auto bg-pal-navy w-16 h-16 rounded-2xl flex items-center justify-center mb-4 shadow-lg shadow-pal-navy/20">
              <GraduationCap className="h-8 w-8 text-pal-beige" />
            </div>
            <h1 className="text-3xl font-bold text-pal-navy tracking-tight">Teacher Portal</h1>
            <p className="text-pal-teal mt-2">Sign in to manage your broadcasts</p>
          </div>
          ...
          <div className="mt-8 text-center text-pal-teal/60 text-sm">
            Don't have an account? <Link href="/signup/teacher" className="text-pal-blue hover:text-pal-navy font-bold underline">Apply as Teacher</Link>
          </div>
        </div>
      </motion.div>
    </div>
